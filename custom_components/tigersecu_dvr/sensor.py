...
async def async_setup_entry(
    ...
    for disk_id in dvr.coordinator.data.get("disks", {}):
        ...
        sensors.append(
            DiskSensor(dvr, disk_id, "available_gb", "Available", "mdi:database-check", "GB")
        )
        for attr_id in dvr.coordinator.data["disks"][disk_id].get("smart_attributes", {}):
...
class DiskSensor(TigersecuSensorBase):
    ...
    def __init__(
        self, dvr: TigersecuDVR, disk_id: int, key: str, name: str, icon: str, unit: str | None = None
    ):
    ...
    def native_value(self) -> str | int | float | None:
        """Return the state of the sensor."""
        return self.coordinator.data.get("disks", {}).get(self._disk_id, {}).get(self._key)
